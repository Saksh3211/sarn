...
function isSarnDocument(doc) {
    if (!doc) return false;
    return doc.fileName.toLowerCase().endsWith('.sarn');
}
...
function getCompilerPath() {
    ...
    if (folders) {
        for (const f of folders) {
            const p = path.join(f.uri.fsPath, 'build', 'compiler', 'sarnc.exe');
...
/**
 * Read sarn.theme.* settings and write them into editor.tokenColorCustomizations
 * so they apply immediately on top of whatever theme the user has active.
 */
function applyThemeCustomizations() {
    const cfg = vscode.workspace.getConfiguration('sarn.theme');
    ...
    const textMateRules = [
        rule('keyword.other.directive.sarn',            'directives'),
        ...
        rule(['support.class.module.sarn',
              'support.function.builtin.sarn'],         'modules'),
        rule('entity.name.function.sarn',               'functions'),
        rule('entity.name.function.call.sarn',          'functionCalls'),
        rule(['string.quoted.double.sarn',
              'string.quoted.single.sarn'],             'strings'),
        rule(['constant.numeric.integer.sarn',
              'constant.numeric.float.sarn'],           'numbers'),
        rule(['comment.line.double-dash.sarn',
              'comment.block.sarn'],                    'comments'),
        rule(['keyword.operator.comparison.sarn',
              'keyword.operator.arithmetic.sarn',
              'keyword.operator.concat.sarn',
              'keyword.operator.arrow.sarn',
              'keyword.operator.bitwise.sarn',
              'keyword.operator.assignment.sarn'],      'operators'),
        rule('entity.name.type.sarn',                   'typeNames'),
    ];
...
/**
 * Remove Sarn token customizations from editor.tokenColorCustomizations
 */
function resetThemeCustomizations() {
    ...
    vscode.window.showInformationMessage('Sarn: Theme customizations cleared.');
}
...
<title>Sarn Theme Customizer</title>
<style>
...
<body>
<h1>🎨 Sarn Theme Customizer</h1>
<p class="subtitle">Changes apply instantly to all open .sarn files. Settings are saved to your VS Code global config.</p>
  ...
  function resetAll() {
    ...
    panel.webview.onDidReceiveMessage(msg => {
        if (msg.command === 'applyTheme') {
            const sarn = vscode.workspace.getConfiguration('sarn.theme');
            for (const [key, val] of Object.entries(msg.changes)) {
                sarn.update(`${key}.color`,     val.color,     vscode.ConfigurationTarget.Global);
                sarn.update(`${key}.fontStyle`, val.fontStyle, vscode.ConfigurationTarget.Global);
            }
...
function runLintOnFile(document) {
    if (!isSarnDocument(document)) return;
    const compiler = getCompilerPath();
    const cwd      = getProjectRoot() || path.dirname(document.uri.fsPath);
    const tmpOut   = path.join(os.tmpdir(), `sarnc_lint_${Date.now()}.ll`);
...
function runLintOnContent(document) {
    ...
    const compiler = getCompilerPath();
    const cwd      = getProjectRoot() || path.dirname(document.uri.fsPath);
    const tmpIn    = path.join(os.tmpdir(), `sarnc_in_${Date.now()}.sarn`);
    const tmpOut   = path.join(os.tmpdir(), `sarnc_out_${Date.now()}.ll`);
...
class sarnHoverProvider {
    provideHover(document, position) {
        ...
        if (wordRange.start.character > 0 && lineText[wordRange.start.character - 1] === '.') {
            ...
            if (modMatch && MODULE_METHODS[modMatch[1]]) {
                ...
                if (method) {
                    ...
                    md.appendCodeblock(`${modMatch[1]}.${method.sig}`, 'sarn');
                    md.appendMarkdown('\n\n' + method.doc);
        ...
        if (kwInfo[word])
            return new vscode.Hover(new vscode.MarkdownString(`**Sarn** — ${kwInfo[word]}`), wordRange);
...
function cmdRunFile() {
    ...
    if (!editor || !isSarnDocument(editor.document)) {
        vscode.window.showErrorMessage('Sarn: No .sarn file is active.');
        return;
    }
...
function cmdBuildCompiler() {
    ...
    let t = vscode.window.terminals.find(x => x.name === 'Sarn Build');
    if (!t) t = vscode.window.createTerminal({ name: 'Sarn Build', cwd:root, shellPath:'cmd.exe' });
    t.show(true);
    t.sendText(`"${bat}"`, true);
}
...
function cmdRunDiagnostics() {
    ...
    if (!doc || !isSarnDocument(doc)) { vscode.window.showErrorMessage('Sarn: No .sarn file.'); return; }
    runLintOnFile(doc);
    vscode.window.showInformationMessage('Sarn: Running diagnostics…');
}

function cmdEmitAST() {
    const doc = vscode.window.activeTextEditor?.document;
    if (!doc || !isSarnDocument(doc)) return;
    doc.save().then(() => {
        const compiler = getCompilerPath();
        const root     = getProjectRoot();
        let t = vscode.window.terminals.find(x => x.name === 'Sarn AST');
        if (!t) t = vscode.window.createTerminal({ name: 'Sarn AST', cwd:root });
        t.show(true);
...
function cmdEmitIR() {
    ...
    doc.save().then(() => {
        const compiler = getCompilerPath();
        const root     = getProjectRoot();
        const outLL    = doc.uri.fsPath.replace(/\.sarn$/i, '.ll');
        let t = vscode.window.terminals.find(x => x.name === 'Sarn IR');
        if (!t) t = vscode.window.createTerminal({ name: 'Sarn IR', cwd:root });
        t.show(true);
...
function cmdResetTheme() {
    // Reset all sarn.theme.* settings to defaults
    const cfg = vscode.workspace.getConfiguration('sarn.theme');
    ...
    vscode.window.showInformationMessage('Sarn: Theme reset to defaults.');
}
...
function activate(context) {
    ...
    diagnosticCollection = vscode.languages.createDiagnosticCollection('sarn');
    context.subscriptions.push(diagnosticCollection);

    // Apply theme customizations on startup
    applyThemeCustomizations();

    // Re-apply when sarn.theme.* settings change
    context.subscriptions.push(
        vscode.workspace.onDidChangeConfiguration(e => {
            if (e.affectsConfiguration('sarn.theme')) {
                applyThemeCustomizations();
            }
        })
    );
    ...
    context.subscriptions.push(
        vscode.workspace.onDidOpenTextDocument(doc => {
            if (isSarnDocument(doc)) runLintOnFile(doc);
        })
    );
    context.subscriptions.push(
        vscode.workspace.onDidSaveTextDocument(doc => {
            const cfg = vscode.workspace.getConfiguration('sarn');
            if (isSarnDocument(doc) && cfg.get('lintOnSave', true)) runLintOnFile(doc);
        })
    );
    context.subscriptions.push(
        vscode.workspace.onDidChangeTextDocument(ev => {
            if (!isSarnDocument(ev.document)) return;
            const cfg   = vscode.workspace.getConfiguration('sarn');
    ...
    // Commands
    context.subscriptions.push(vscode.commands.registerCommand('sarn.runFile',        cmdRunFile));
    context.subscriptions.push(vscode.commands.registerCommand('sarn.buildCompiler',  cmdBuildCompiler));
    context.subscriptions.push(vscode.commands.registerCommand('sarn.compile',        cmdRunDiagnostics));
    context.subscriptions.push(vscode.commands.registerCommand('sarn.emitAST',        cmdEmitAST));
    context.subscriptions.push(vscode.commands.registerCommand('sarn.emitIR',         cmdEmitIR));
    context.subscriptions.push(vscode.commands.registerCommand('sarn.customizeTheme', () => openThemeCustomizer(context)));
    context.subscriptions.push(vscode.commands.registerCommand('sarn.resetTheme',     cmdResetTheme));
