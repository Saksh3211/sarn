...
2. `types.sarn` - Type annotations and inference
3. `functions.sarn` - Function definitions and calls
4. `control_flow.sarn` - if/elseif/else statements
5. `loops.sarn` - For and while loops
6. `loop_control.sarn` - break and continue
7. `tables.sarn` - Arrays and maps
8. `strings.sarn` - String literals and concatenation
9. `operators.sarn` - Arithmetic, comparison, logic
10. `logic.sarn` - Logical operators and expressions
11. `scoping.sarn` - Variable scope and shadowing
12. `defer.sarn` - Defer statements for cleanup
13. `multiple_return.sarn` - Functions with multiple returns

**stdlib/** - Standard library usage
1. `math.sarn` - Math module (sqrt, pow, sin, cos, etc.)
2. `string.sarn` - String manipulation functions
3. `io.sarn` - Input/output with colors
4. `os.sarn` - Operating system functions
5. `fs_path.sarn` - Filesystem and path operations
6. `json.sarn` - JSON parsing and encoding
7. `random_datetime.sarn` - Random numbers and datetime
8. `crypto.sarn` - Cryptography and hashing
9. `reflection.sarn` - Type checking and conversion (stdata module)
10. `table_ops.sarn` - Table manipulation functions

**advanced/** - Advanced language features
1. `memory.sarn` - Manual memory management (alloc, free, deref)
2. `pointers.sarn` - Pointer operations and dereferencing
3. `casting.sarn` - Type casting and conversions
4. `records.sarn` - Record types (structs)
5. `enums.sarn` - Enum types
6. `error_handling.sarn` - panic() and error handling
7. `oop.sarn` - Object-oriented patterns with methods
8. `union_types.sarn` - Union type declarations
9. `multiple_returns.sarn` - Advanced return values
10. `recursion.sarn` - Recursive functions
11. `defer_order.sarn` - Multiple defer statements

**graphics/** - Graphics and UI
1. `demo_3d.sarn` - 3D rendering with Raylib
2. `fonts.sarn` - Font loading and text rendering
3. `ui_demo.sarn` - Interactive UI components

---

## File directives

Every `.sarn` file should start with directives on line 1+.
```
--!!type:strict
--!!type:nonstrict
--!!mem:man
...
import ("utils.sarn")     -- file import (path relative to current file)
```
...
You can create reusable modules by splitting code into separate `.sarn` files. Each file acts as a package:
...
function main(): int
    -- add is not exported, only accessible within mymath.sarn
...
```lua
path.join(a, b)
path.basename(p)    -- "file.sarn"
path.dirname(p)     -- "examples/portfolio"
path.extension(p)   -- ".sarn"
path.stem(p)        -- "file"
path.absolute(p)
path.normalize(p)
path.exists(p)      -- int
...
```powershell
.\sarn.ps1 Sarn-Run examples\hello.sarn
```
