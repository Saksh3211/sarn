...
1. **basics/** - Start here
   - `variables.sarn` - Declare variables, types
   - `functions.sarn` - Create and call functions
...
2. **stdlib/** - Use the standard library
   - `math.sarn` - Math functions
   - `io.sarn` - Input/output
   - `fs_path.sarn` - File operations
   - `json.sarn` - JSON handling

3. **advanced/** - Advanced patterns
   - `memory.sarn` - Manual memory management
   - `records.sarn` - Define record types
   - `oop.sarn` - Object-oriented patterns
   - `recursion.sarn` - Recursive functions

4. **graphics/** - Graphics programming
   - `demo_3d.sarn` - 3D rendering
   - `ui_demo.sarn` - Interactive UI
